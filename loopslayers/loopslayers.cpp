...
void calc_dwc_algs_limits_f32(dwc_algs_limits* out, int C, int W, int H, int kw, int kh, int64_t H0, int64_t W0, int padding_top, int padding_left, int padding_bottom, int padding_right)
{
        ...
        *out = loops::DepthwiseconvGenerator(CTX).calc_dwc_algs_limits(C, W, H, kw, kh, H0, W0, padding_top, padding_left, padding_bottom, padding_right);
        return;
#endif
}
...
maxpool_f32_t generate_maxpool_f32(int kh, int kw, int padding_top, int padding_left, int padding_bottom, int padding_right, int activation_type, float alpha)
{
    ...
    if(kh <= 0 || kw <= 0 || padding_top < 0 || padding_left < 0 || padding_bottom < 0 || padding_right < 0 || kh * kw > 39*39 || 
       (activation_type != ACT_NONE && activation_type != ACT_RELU && activation_type != ACT_RELU6 && activation_type != ACT_LRELU))
        return 0;
    return loops::MaxpoolGenerator(CTX).generate(kh, kw, padding_top, padding_left, padding_bottom, padding_right, activation_type, alpha);
#endif 
}
...
void calc_maxpool_algs_limits_f32(maxpool_algs_limits* out, int C, int W, int H, int kw, int kh, int64_t H0, int64_t W0, int padding_top, int padding_left, int padding_bottom, int padding_right)
{
        ...
        *out = loops::MaxpoolGenerator(CTX).calc_maxpool_algs_limits(C, W, H, kw, kh, H0, W0, padding_top, padding_left, padding_bottom, padding_right);
        return;
#endif
}
