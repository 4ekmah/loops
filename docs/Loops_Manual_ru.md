...
Возможность собирать нужный код на лету, позволяет собирать оптимальные ядра для любого набора параметров(например, слои нейронных сетей), а также склеивать циклы ядер, когда уже известно, в какой последовательности они будут вызываться, что позволяет избежать лишних прокачек данных через кэш. Иными словами, основное назначение проекта - ручная оптимизация.<br/><br/>
...
* **USE_CONTEXT_(ctx)** - активирует систему макросов в текуще базовом блоке, привязывая её к текущему контексту.
