...
void DepthwiseconvTest::run() 
{
    ...
    std::vector< std::pair<std::vector<int>, std::vector<std::vector<int> > > > fixtures = {
        {{3,3}, { {32, 200, 200},  {256, 40, 40} } } , 
        {{5,5}, { {32, 200, 200},  {256, 40, 40} } } , 
        {{7,7}, { {32, 200, 200},  {256, 40, 40} } } , 
        {{9,9}, { {32, 200, 200},  {256, 40, 40} } }
//      {{kh,kw}, { {C, H, W},  ... } }
    };
    for(auto gfxt: fixtures)
    {
        int kh = gfxt.first[0];
        int kw = gfxt.first[1];
        dwconv_t func = generate(kh, kw);
        for(auto fxt: gfxt.second)
        {
            const int C = fxt[0];
            const int H = fxt[1];
            const int W = fxt[2];
...
DepthwiseconvTest::dwconv_t DepthwiseconvTest::generate(int kh, int kw)
{
    ...
    IReg data, kernel, bias, H, W, C, result, H0, W0;
    USE_CONTEXT_(CTX);
    STARTFUNC_(funcname, &data, &kernel, &bias, &H, &W, &C, &result, &H0, &W0)
    {
        ...
        WHILE_(channel < C)
        {
            ...
            WHILE_(y < H0)
            {
                IReg xvectorend = select( y >= yvectorend, xvectorend_, W0); //x < xmiddle must be handled by scalar code, and x >= xmiddle by vector code
                ...
                WHILE_(x < W0)
                {
                    ...
                    for(int krow = 0; krow < kh; krow++) 
                    {
                        ...
                        {
                            const int kerelemnum = krow*kw + kcol;
                            VReg<float> justloaded = loadlane<float>(data__ + kcol*elemsize, 0);
                            vres = fma(vres, justloaded, vkernel[kerelemnum/CTX.vlanes<float>()], kerelemnum%CTX.vlanes<float>());
                        }
                        if(krow + 1 < kh)
                            data__ += W * elemsize;
                    }
                    storelane<float>(result_rs + x * elemsize, vres, 0);                    
                    x += 1;
                }                
                y += 1;
            }
...
void DepthwiseconvTest::ref(float* data, float* kernel, float* bias, int H, int W, int C, float* result, int H0, int W0, int kh, int kw)
{
        ...
        for(int i = 0; i < H; i++)
            for(int j = 0; j < W; j++)
            {
                ...
                if(tchk != r)
                {
                    ...
                    float relErr = absErr / std::max(std::abs(tchk), std::abs(r));
